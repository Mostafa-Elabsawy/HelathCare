...
export class BookAppointmentComponent implements OnInit {
  ...
  private mapApiDoctor(api: DoctorProfileResponseAPI): Doctor {
    return {
      ...
      specialty: api.specialty || 'General Practice',
      medicalLevel: api.medicalLevel || 'Specialist',
      gender: api.gender === 'male' || api.gender === 'female' ? api.gender : 'male',
      workingDays: api.workingDay ?? [],
      workingHourStart: api.workingHourStart ?? '09:00',
      workingHourEnd: api.workingHourEnd ?? '17:00',
      price: api.price ?? 400,
      rating: api.rate ?? 4.5,
      reviewsCount: 0,
      governorate: api.governorate || 'Cairo',
